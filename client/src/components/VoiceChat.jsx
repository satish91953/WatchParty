            ...
            {peers.length > 0 && (
              <div style={{ 
                ...
                {peers.map((peerObj) => {
                  ...
                  // Find user info from users array (peerId is socket.id, users have userId which is also socket.id)
                  const peerUser = users.find(u => u.userId === peerId) || { username: `Guest-${peerId.substring(0, 6)}` };
